use abm::{DoubleHifiProblemBuilder, Symmetry, utility::diagonalize};
...
use crate::{
    ...
    utility::{
        AngularPair, AnisoHifi, GammaSpinRot, aniso_hifi_tram_mel, create_angular_pairs,
        percival_coef_tram_mel, singlet_projection_uncoupled, spin_rot_tram_mel,
    ...
    V: SimplePotential,
{
    ...
    pub fn build(
    ...
    ) -> AlkaliRotorAtomProblem<TramStates, P, V> {
        use TramStates::*;
        ...
        assert!(hifi_problem.second.s == hu32!(1 / 2) || hifi_problem.second.s == hu32!(0));
        ...
        let angular_blocks = angular_block_basis.iter()
            .map(|(l, basis)| {
                ...
                let mut zeeman_prop = operator_diagonal_mel!(&basis, |[s_rotor: RotorS, s_atom: AtomS]| {
        ...
        let singlet_potentials = self
            ...
            .map(|(lambda, pot)| {
                let masking_singlet = operator_mel!(&ordered_basis,
                    |[ang: Angular, n_tot: NTot, s_rotor: RotorS, s_atom: AtomS]| {
    ...
    fn basis(
        ...
        let i_atom = into_variant(get_spin_basis(hifi_problem.second.i), AtomI);
    ...
    V: SimplePotential + Clone,
{
    fn scattering_for(
        ...
        let mut triplets_iter = self.triplets.iter().map(|(p, m)| {
            let masking = states.transpose() * m * states.as_ref();
            MaskedPotential::new(p.clone(), masking)
        });
        let mut triplets =
            Composite::new(triplets_iter.next().expect("No triplet potentials found"));
        for p in triplets_iter {
            triplets.add_potential(p);
        }

        let mut singlets_iter = self.singlets.iter().map(|(p, m)| {
            let masking = states.transpose() * m * states.as_ref();
            MaskedPotential::new(p.clone(), masking)
        });
        let mut singlets =
            Composite::new(singlets_iter.next().expect("No triplet potentials found"));
