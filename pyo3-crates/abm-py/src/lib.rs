use abm::{
    ABMHifiProblem, ABMProblemBuilder, ABMVibrational, DoubleHifiProblemBuilder,
    HifiProblemBuilder, Symmetry,
    abm_states::{ABMStates, HifiStates},
};
...
impl HifiProblemBuilderPy {
    ...
    pub fn new(s: u32, i: u32) -> Self {
        Self(HifiProblemBuilder::new(
            HalfU32::from_doubled(s),
            HalfU32::from_doubled(i),
        ))
    }
...
impl DoubleHifiProblemBuilderPy {
    ...
    pub fn new(first: PyRef<HifiProblemBuilderPy>, second: PyRef<HifiProblemBuilderPy>) -> Self {
        Self(DoubleHifiProblemBuilder::new(
            first.0.clone(),
            second.0.clone(),
        ))
    }
    ...
    pub fn new_homo(single: PyRef<HifiProblemBuilderPy>, symmetry: &str) -> Self {
        ...
        Self(DoubleHifiProblemBuilder::new_homo(
            single.0.clone(),
            symmetry,
        ))
    }
...
impl ABMProblemBuilderPy {
    ...
    pub fn with_vibrational(
        ...
        assert!(
            singlet_states.len().pow(2) == fc_factors.len(),
            "non-compatible franck-condon factor matrix size"
        );

        let singlet_states: Vec<Energy<Au>> =
            singlet_states.iter().map(|&x| Energy(x, Au)).collect();
        let triplet_states: Vec<Energy<Au>> =
            triplet_states.iter().map(|&x| Energy(x, Au)).collect();
