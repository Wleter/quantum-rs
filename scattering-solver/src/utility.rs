use faer::{
    ...
    linalg::{
        cholesky::bunch_kaufman::compute::{cholesky_in_place, cholesky_in_place_req},
        lu::{
            self,
            partial_pivoting::{compute::lu_in_place_req, inverse::invert_req},
        },
...
pub fn inverse_symmetric_inplace(
    ...
    zipped!(mat_temp.as_mut(), mat).for_each(|unzipped!(o, m)| *o = *m);
    ...
    cholesky_in_place(
        ...
        faer::Parallelism::None,
        PodStack::new(&mut GlobalPodBuffer::new(
            cholesky_in_place_req::<usize, f64>(dim, Parallelism::None, Default::default())
                .unwrap(),
        )),
        Default::default(),
    );
